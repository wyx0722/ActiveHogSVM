...
for tt = 1:n_trials
     ...
     save(sprintf('DenseMBH_Rochester/denseMBH_%sS%iR%i_head.mat',act_list{aa},ss,tt),'features_head');
     clear features_head
     
     features_torso = collect_features('Dataset_RochesterADL', 'torso', ss,act_list{aa},tt);
     save(sprintf('DenseMBH_Rochester/denseMBH_%sS%iR%i_torso.mat',act_list{aa},ss,tt),'features_torso');
