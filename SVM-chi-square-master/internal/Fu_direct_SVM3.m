 function [accuracy,prob_estimates,bestmodel,  class_label, meta_res]=Fu_direct_SVM3(Xtrain, Xtest, train_video_label,  test_video_label, opt)
...
%
% this version is changed from Fu_direct_SVM2.m,
%
% change-list:
%                   add the support for extensional distance support for SVM (we can use cosine similarity as the distance
%                   metric).
%
if nargin<5
    opt = struct;
...
opt=getPrmDflt(opt, {'norm_type','row','permutation',0,'kernel','chi-square','distance_type','chi-square','nFold',3,'outProb', 1,'useWeight', true},-1);
global iif;
...
% opt.kernel, using default distance type:
if strcmp(opt.kernel,'RBF')&&(~isfield(opt,'distance_type'))
   [c,g,bestcv,acc,cmat,bestmodel] = Fu_libsvm_cv(Ytrain,train_label,opt);
    [class_label, accuracy,prob_estimates] = svmpredict(test_label, ...
                                Ytest, bestmodel, '-b 1');
elseif strcmp(opt.kernel,'linear') &&(~isfield(opt,'distance_type'))
     [c,g,bestcv,acc,cmat,bestmodel] = Fu_libsvm_cv(Ytrain,train_label,opt);
    [class_label, accuracy,prob_estimates] = svmpredict(test_label, ...
                                Ytest, bestmodel, '-b 1');
else     % if strcmp(opt.kernel,'chi-square')||strcmp(opt.kernel,'chisq')||strcmp(opt.kernel,'nrmcorr')
    opt.customKernel = 1; 
    % to invoke when cross validataion of a subset of data
    opt.makeKernelPtr =@Fu_compute_kernel_matrices_v2; 
    [Ktrain, Ktest] = Fu_compute_kernel_matrices_v2(Ytrain, Ytest,opt,opt.distance_type);
