...
deepmodel = {};
W = option.hyperfeatures.encoding_W; % the receptive field size W is counted on features
S = option.hyperfeatures.encoding_S; % stride is also depending on features.
...
%         ydata = tsne(full(codebook));
%         figure(1);scatter(ydata(:,1),ydata(:,2));drawnow;
%     end
...
for ii = 1:NN
    ...
    if W>=Nf
        ...
        for tt = 1:S:Nf-W
    %         TT = min(tt+W-1,Nf);
            tmp = sum(stip_src{ii}.features(tt:tt+W-1,dd:end),1); % sum pooling       
            stip_des{ii}.features(idxx,1:dd-1) = ...
                stip_src{ii}.features(tt,1:dd-1);    
            stip_des{ii}.features(idxx,dd:dd+size(tmp,2)-1) = tmp./(sum(tmp)+1e-6); %l1-normalize

            idxx = idxx+1;
        end
    end
    stip_src{ii}.features = [];
    if ~isfield(stip_des{ii},'features')
        fdafdsaf
    end;
end
