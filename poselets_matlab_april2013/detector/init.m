function config=init(poselet_detector_path);

format compact; format long g;
dbstop if error;
addpath([poselet_detector_path,'/annotation_tools']);
addpath([poselet_detector_path,'/poselet_detection']);
addpath([poselet_detector_path,'/categories']);
addpath([poselet_detector_path,'/visualize']);
addpath([poselet_detector_path,'/pascal']);
...
config.DETECT_MAX_HITS_PER_SCALE_PER_POSELET = inf; % original value is inf
...
config.DATA_DIR = [fileparts(poselet_detector_path),'/data'];
